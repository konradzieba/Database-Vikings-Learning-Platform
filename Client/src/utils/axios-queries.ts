...
export const getTaskInfoByIdQueryFn = async (taskId: number) => {
	const { data } = await axios.get<TGetTaskInfoById>(
		`/tasks/getTaskInfoById/${taskId}`
	);
	return data;
};
...
export const createLessonMutationFn = async (
	createLessonRequest: TCreateLessonRequest
) => {
	const { data } = await axios.post<TMessageResponse>(
...
export const addStudentToGroupMutationFn = async (
...
export const getAbsentStudentsQueryFn = async (lessonId: number) => {
	const { data } = await axios.get<TGetAbsentStudents>(
		`/lessons/getAbsentStudents/${lessonId}`
	);
	return data;
};
...
export const correctLessonFrequencyListMutationFn = async (
	lessonId: number,
	newAbsentStudentList: number[]
) => {
	const { data } = await axios.patch<TMessageResponse>(
		`/lessons/correctLessonFrequency/${lessonId}`,
		{
...
export const getSpecialTaskByIdQueryFn = async (specialTaskId: number) => {
	const { data } = await axios.get<TGetSpecialTaskById>(
		`/tasks/getSpecialTaskInfoById/${specialTaskId}`
	);
